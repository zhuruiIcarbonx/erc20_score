...
type Chain struct {
	...
	SynedScoreTime  time.Time `gorm:"column:syned_score_time"`
}
...
func ChainUpdateSynedScoreTime(db *gorm.DB, chainId string, synedScoreTime time.Time) error {

	err := db.Debug().Model(Chain{}).Where("id = ?", chainId).Updates(Chain{SynedScoreTime: synedScoreTime}).Error
	return err
